...
Imagine 根据 CZI 文件名自动识别通道和标记物。文件名中包含 `标记物-波长` 格式的字段，波长决定通道颜色：
...
```
MIP 80%O2 Day11 WT1 left3 CD45-568 CD31-488 DAPI 2x2x3 2.czi
                           ~~~~~~~~ ~~~~~~~~ ~~~~
                           红色通道   绿色通道  蓝色通道
```
...
```
MIP [条件] [时间] [样本] [位置] [标记1-波长] [标记2-波长] ... [维度] [编号].czi
```
...
| 样本 | `wt1` / `WT1` | 样本标识 |
| 位置 | `left3` / `left4` | 拍摄位置 |
| 标记-波长 | `Ki67-568`, `CD45-568` | 标记物名称 + 荧光波长 |
| 维度 | `2x2x3` | 图像拼接/z-stack 参数 |
| 编号 | `1`, `2`, `3` | 序号 |

> **导入文件后**，左侧参数面板会自动更新通道标签（如 `红色 (CD45)` `绿色 (CD31)`），无需手动配置。
