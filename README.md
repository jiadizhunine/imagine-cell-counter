...
Imagine 采用**深色主题 + 三栏布局**设计，自适应 16:9 屏幕比例，贴合研究者的日常使用习惯：

```
┌─────────────────────────────────────────────────────────────────┐
│  Imagine                     ◀上一张 [3/10▼] 下一张▶    文件名  │
├──────────┬──────────────────────────────────┬───────────────────┤
│  左侧栏   │  通道: Blue Red Green White       │    右侧栏         │
│          │  叠加层: 核轮廓 AT2数量 AT2增殖    │                   │
│ 检测到策略 │  ┌───────────────────────┐      │  分析结果          │
│ 通道分配   │  │                       │      │  ├ 细胞核总数       │
...
│ 绿色阈值   │  │                       │      │                   │
│ 白色阈值   │  └───────────────────────┘      │  批量结果表格       │
│ 环宽度    │  标注: 手动标注 Tag 清除  0个标注  │  (双击跳转/右键删除) │
│ 最小核面积 │                                  │                   │
│ [开始分析] │                                  │  [导出 CSV]        │
└──────────┴──────────────────────────────────┴───────────────────┘
```
...
- **16:9 自适应**：窗口大小根据屏幕分辨率自动调整，16:9 屏幕下比例完美
- **所见即所得**：切换通道、调整叠加层时，图像实时更新且保持当前缩放位置不变
- **符合直觉的操作**：滚轮缩放、左键拖拽平移、右键标注 — 与常见看图软件一致
- **文件列表管理**：点击 `N/M` 按钮弹出文件列表，可快速跳转或关闭指定文件
- **动态自适应**：界面控件根据图片内容自动显示/隐藏，无关参数不会干扰视线
- **通用标记物识别**：不仅识别 Ki67、ProSPC 等已知标记，还能自动解析任意 `标记物-波长` 格式（如 CD45-568、CD31-488）
...
| `GFP` | GFP | 绿色 (488nm) |
| `任意标记-568` | 自动识别 | 红色 (568nm) |
| `任意标记-647` | 自动识别 | 白色 (647nm) |
| `任意标记-488` | 自动识别 | 绿色 (488nm) |
| DAPI | DAPI | 蓝色 (始终存在) |

> **通用解析器**：即使文件名中的标记物不在预设列表中（如 `CD45-568`、`CD31-488`），Imagine 也能自动识别并更新左侧参数面板的通道标签。
...
| **AT1 数量** | HOPX + DAPI | HOPX⁺ 细胞计数（核+环混合检测） |
| **AT1 增殖** | HOPX + Ki67 + DAPI | Ki67⁺HOPX⁺ / HOPX⁺ |

### 核心算法

- **DAPI 核分割**：高斯模糊 → Triangle/Otsu 双算法自动阈值 → 形态学开运算去噪 → 填洞 → 分水岭分离粘连核
- **AT1 混合检测**：考虑 AT1 细胞扁平特殊形态，同时检测核内和窄环胞质信号，取两者最大值与阈值比较，显著提升扁平细胞的识别准确率
- **AT2 胞质环检测**：膨胀核区域生成环形 ROI，计算环内 ProSPC 信号强度
...
- **撤回**：`Ctrl+Z` 撤回上一步标注
- **导出**：通过 `文件 > 导出标注 TIF` 或 `Ctrl+S` 导出为 Fiji 兼容的 TIF 文件（含嵌入 ROI 数据）

### 文件管理

- **文件列表**：点击导航栏中的 `N/M` 按钮弹出完整文件列表
- **快速跳转**：在列表中点击文件名即可跳转
- **关闭文件**：支持关闭当前文件或关闭所有文件
- **Ctrl 多选**：打开文件时支持 Ctrl/Shift 多选
...
| 环宽度 | 15 px | 胞质信号检测的环形区域宽度（AT1 自动使用半值） |
...
- **图像处理**: scikit-image (分水岭分割, 形态学), scipy (距离变换, 标记, 中值滤波)
- **数值计算**: NumPy (向量化运算)
...
- **AT1 算法改进**：采用核+环混合检测方案，适应 AT1 细胞扁平形态，显著提升识别准确率
- **Ki67 去噪**：增加中值滤波预处理，有效减少染色杂点和浮色导致的假阳性
- **16:9 屏幕适配**：窗口大小根据屏幕分辨率自适应，16:9 屏幕下比例正确
- **通用标记物解析**：支持任意 `标记物-波长` 格式的文件名解析（如 CD45-568、CD31-488），参数面板自动更新
- **文件列表管理**：导航栏 `N/M` 按钮可展开文件列表，支持快速跳转和关闭文件
- **标注工具栏优化**：标注控件移至图像下方独立工具栏，操作更便捷
- **导出 TIF 归位**：导出标注 TIF 功能移至 `文件` 菜单栏
